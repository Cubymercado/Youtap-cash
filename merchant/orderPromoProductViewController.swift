//
//  orderPromoProductViewController.swift
//  Youtap Merchant
//
...
class orderPromoProductViewController: UIViewController {

    // Variables
    var product: suppliersPromos?
    ...
    static func fromStoryboard(_ storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: nil)) -> orderPromoProductViewController {
        let controller = storyboard.instantiateViewController(withIdentifier: "orderPromoProductViewController") as! orderPromoProductViewController
        return controller
    }
    ...
    @IBAction func addToProducts(_ sender: Any) {
        ...
        if (product?.name != nil) && pricePrice != "" {
            if constantCart.saveObject(product: (product?.name)!, price: (product?.unitPrice)!, inventory: Int(inventoryAmount)!, productImage: picture as NSData, productDescription: (product?.size)!, documentID: "testProductPromo" ) {
