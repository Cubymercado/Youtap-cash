...
class productQuantityViewController: UIViewController, UITextFieldDelegate {
    ...
    func showMeTheGoodiesTwo(entry: Cart) {
       currency = global.appMainCurrency ?? "NZD"
        ...
        let money = "\(currency) \(xNSNumberP.stringValue)"
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        let money = "\(currency) \(newPriceText.text!)"
...
extension productQuantityViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let priceInt = Float(amountForCode)!
        let percentageInt = Float(percentageAmounts[indexPath.row])!
        let result = priceInt * percentageInt
        let possumMoney = priceInt - Float(result)
        let resultString = possumMoney as NSNumber
        productAmountLabel.text = "\(currency) \(resultString.stringValue)"
    }
}
